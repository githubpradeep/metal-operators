...
songs via Metal KNN (50K corpus + 10K queries × D=8 × K=5) and compares
speed vs sklearn NearestNeighbors.

For the Rust BLAS baseline (Criterion) this shape gives 22× speedup:
  cargo bench --bench knn_benchmark -- "Q=10K_C=50K_D=8_K=5"
"""
import time
import numpy as np
...
titles = [f"Song {i} — {rng.choice(genres)} #{rng.integers(1, 100)}" for i in range(N_CORPUS)]
...
print(f"\n{'Query #':>8} | {'Nearest neighbours (index → title)':<55}")
print("-" * 70)
for q in range(5):
    nbrs = ", ".join(f"{i} → {titles[i][:25]}" for i in indices[q])
    print(f"  q={q:>4}  | {nbrs}")
...
try:
    ...
    sk.fit(corpus)
    _, _ = sk.kneighbors(queries)
    t1 = time.perf_counter()
    sk_ms = (t1 - t0) * 1000

    print(f"\n{'Method':<35} {'Time (ms)':>10} {'Speedup':>10}")
    print("-" * 60)
    print(f"{'Metal GPU (this library)':<35} {metal_ms:>10.1f} {'—':>10}")
    print(f"{'sklearn NearestNeighbors (CPU)':<35} {sk_ms:>10.1f} {sk_ms / metal_ms:>9.1f}×")
    print(f"\nNote: the Rust Criterion benchmark shows 22× vs BLAS for this shape:")
    print(f"  cargo bench --bench knn_benchmark -- \"Q=10K_C=50K_D=8_K=5\"")
except ImportError:
