...
fn test_knn_debug11() {
    ...
    // Test with k param + heap_insert — INDIVIDUAL set_bytes
    let source = r#"
    ...
    let pipeline = ctx.compile_kernel(source, "knn_test11").expect("compile");

    let qbuf = ctx.new_buffer(&queries);
    let cbuf = ctx.new_buffer(&corpus);
    let nq_buf = ctx.new_buffer(&norms_q);
    let nc_buf = ctx.new_buffer(&norms_c);
    let scores = ctx.new_buffer_uninitialized((nq * k * 4) as u64);
    let indices = ctx.new_buffer_uninitialized((nq * k * 4) as u64);
    ...
    enc.set_buffer(3, Some(&indices), 0);
    enc.set_buffer(4, Some(&nq_buf), 0);
    enc.set_buffer(5, Some(&nc_buf), 0);
    let nq_v = nq as u32; let nc_v = nc as u32; let d_v = d as u32; let k_v = k as u32;
    enc.set_bytes(6, 4, ptr::from_ref(&nq_v).cast());
    enc.set_bytes(7, 4, ptr::from_ref(&nc_v).cast());
    enc.set_bytes(8, 4, ptr::from_ref(&d_v).cast());
    enc.set_bytes(9, 4, ptr::from_ref(&k_v).cast());
    enc.dispatch_thread_groups(MTLSize { width: 1, height: 1, depth: 1 }, MTLSize { width: 128, height: 1, depth: 1 });
    enc.end_encoding();
    cmd_buf.commit();
    cmd_buf.wait_until_completed();

    let gpu_scores: Vec<f32> = ctx.read_buffer(&scores, nq * k);
    let gpu_idx: Vec<u32> = ctx.read_buffer(&indices, nq * k);
    eprintln!("scores: {:?}", gpu_scores);
    eprintln!("idxs: {:?}", gpu_idx);
    ...
    // Convert GPU scores to true distances
    ...
    eprintln!("gpu true: {:?}", gpu_d);

    for q in 0..nq {
        for j in 0..k {
            assert_eq!(gpu_idx[q*k+j], cpu_i[q*k+j], "idx q={} j={}", q, j);
